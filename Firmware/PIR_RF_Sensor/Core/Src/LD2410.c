...
void sendCommand(char *commandStr, char *commandValue, uint16_t commandValueLen)
{
	//create frame with specific length
	uint8_t frame[10+commandValueLen];
	...
	uint8_t frameCommand[] = {commandStr[0], commandStr[1]};
	// command value bytes
	uint8_t frameCommandValue = malloc(commandValueLen * sizeof(char));
	memcpy(frameCommandValue, commandValue, commandValueLen * sizeof(char));
	// frame end bytes
